...
interface GTDState {
  ...
  updateWaitingFor: (
    id: string,
    updates: Partial<Omit<WaitingFor, 'id' | 'createdAt'>>,
  ) => Promise<void>
  ...
  updateSomedayMaybe: (
    id: string,
    updates: Partial<Omit<SomedayMaybe, 'id' | 'createdAt'>>,
  ) => Promise<void>
...
export const useGTDStore = create<GTDState>()((set, get) => ({
  ...
  addWaitingFor: async (
    ...
    try {
      const { item } = await api.addWaitingFor(
        content,
        person,
        projectId,
        expectedDate,
  ...
  moveSomedayToAction: async (id, context, projectId = null) => {
    ...
    try {
      ...
      const { item: newAction } = await api.addAction(
        item.content,
        context,
        projectId,
        null,
  ...
  moveSomedayToWaitingFor: async (id, person, expectedDate = null) => {
    ...
    try {
      ...
      const { item: newWaitingFor } = await api.addWaitingFor(
        item.content,
        person,
        null,
        expectedDate,
