...
export function CalendarView() {
  ...
  const selectedDateActions = selectedDate
    ? actionsWithDueDate.filter(
        (a) => a.dueDate && isSameDay(a.dueDate, selectedDate),
      )
    : []

  const selectedDateWaiting = selectedDate
    ? waitingWithDate.filter(
        (w) => w.expectedDate && isSameDay(w.expectedDate, selectedDate),
      )
    : []
  ...
  return (
    <div className="flex flex-col h-full p-6 gap-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">View your actions and deadlines</p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="flex justify-center">
            <Calendar
              ...
              modifiers={{
                hasItems: (date) =>
                  datesWithItems.has(format(date, 'yyyy-MM-dd')),
              }}
              modifiersStyles={{
                hasItems: {
                  fontWeight: 'bold',
