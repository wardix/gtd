...
import { SOMEDAY_CATEGORIES, CONTEXTS, type SomedayCategory, type Context, type SomedayMaybe as SomedayMaybeType } from '@/types/gtd';
...
export function SomedayMaybe() {
    ...
    const [selectedItem, setSelectedItem] = useState<SomedayMaybeType | null>(null);
    ...
    const [activeCategory, setActiveCategory] = useState<SomedayCategory | 'all'>('all');
    ...
    const moveSomedayToWaitingFor = useGTDStore((state) => state.moveSomedayToWaitingFor);
    ...
    return (
        <div className="flex flex-col h-full p-6 gap-6">
                ...
                <Dialog open={newItemOpen} onOpenChange={setNewItemOpen}>
                    ...
                    <DialogContent>
                        ...
                        <div className="space-y-4">
                            ...
                            />
                            <Select value={itemCategory} onValueChange={(v) => setItemCategory(v as SomedayCategory)}>
                                <SelectTrigger>
            ...
            {/* Category Tabs */}
            <Tabs value={activeCategory} onValueChange={(v) => setActiveCategory(v as SomedayCategory | 'all')}>
                <TabsList>
                    <TabsTrigger value="all">
                        All
                        <Badge variant="outline" className="ml-1.5">{somedayMaybe.length}</Badge>
                    </TabsTrigger>
                    {SOMEDAY_CATEGORIES.map((cat) => {
                        const count = somedayMaybe.filter((i) => i.category === cat.value).length;
                        return (
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex gap-2 mt-1.5">
                                                        <Badge variant="secondary" className="text-xs">
                                                            {SOMEDAY_CATEGORIES.find((c) => c.value === item.category)?.label}
                                                        </Badge>
                                                    </div>
            ...
            <Dialog open={moveDialogOpen} onOpenChange={setMoveDialogOpen}>
                <DialogContent>
                    ...
                    <div className="space-y-4">
                        ...
                        <Select value={moveContext} onValueChange={(v) => setMoveContext(v as Context)}>
                            <SelectTrigger>
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleMoveToAction}>
                            Move to Actions
                        </Button>
                    </DialogFooter>
                </DialogContent>
            ...
            <Dialog open={delegateDialogOpen} onOpenChange={setDelegateDialogOpen}>
                <DialogContent>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="text-sm font-medium mb-2 block">Who will handle this?</label>
                            <Input
                        ...
                        <div>
                            <p className="text-sm font-medium mb-2">Expected Date (optional)</p>
                            <Calendar
                                mode="single"
