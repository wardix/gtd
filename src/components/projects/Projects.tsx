...
export function Projects() {
  ...
  const toggleActionComplete = useGTDStore(
    (state) => state.toggleActionComplete,
  )
  ...
  >({})
  const [newActionContext, setNewActionContext] = useState<
    Record<string, Context>
  >({})
  const [newWaitingContent, setNewWaitingContent] = useState<
    Record<string, string>
  >({})
  const [newWaitingPerson, setNewWaitingPerson] = useState<
    Record<string, string>
  >({})
                  ...
                  <CardContent>
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-1.5">
                        <StatusIcon
                          className={`h-4 w-4 ${STATUS_CONFIG[project.status].color}`}
                        />
                    ...
                    <div className="flex gap-1 mt-3">
                      {(
                        ['active', 'on-hold', 'completed'] as ProjectStatus[]
                      ).map((status) => (
                        <Button
                          key={status}
                          size="sm"
                          variant={
                            project.status === status ? 'secondary' : 'ghost'
                          }
                    ...
                    <div className="mt-4 pt-3 border-t">
                      <div className="flex gap-2">
                        <Input
                          ...
                          onChange={(e) =>
                            setNewActionContent((prev) => ({
                              ...prev,
                              [project.id]: e.target.value,
                            }))
                          }
                          onKeyDown={(e) =>
                            e.key === 'Enter' && handleAddAction(project.id)
                          }
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            setNewActionContext((prev) => ({
                              ...prev,
                              [project.id]: v as Context,
                            }))
                          }
                        >
                    ...
                    <div className="mt-3 pt-3 border-t">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setNewWaitingPerson((prev) => ({
                              ...prev,
                              [project.id]: e.target.value,
                            }))
                          }
                          onKeyDown={(e) =>
                            e.key === 'Enter' && handleAddWaitingFor(project.id)
                          }
                        ...
                        <Button
                          ...
                          disabled={
                            !newWaitingContent[project.id]?.trim() ||
                            !newWaitingPerson[project.id]?.trim()
                          }
                        >
                          <Plus className="h-4 w-4" />
                        </Button>
