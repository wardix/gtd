...
waitingFor.get('/', async (c) => {
    try {
        ...
        const items = await db.collection('waitingFor')
            .find({ userId })
            .sort({ createdAt: -1 })
            .toArray();
...
waitingFor.post('/', async (c) => {
    try {
        const { userId } = getAuth(c);
        const { content, person, projectId = null, expectedDate = null } = await c.req.json();
...
waitingFor.patch('/:id', async (c) => {
    try {
        ...
        const db = getDB();
        const result = await db.collection('waitingFor').findOneAndUpdate(
